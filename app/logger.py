...
def set_logger(log_level=20):
    ...
    my_format = "[%(levelname).4s] %(asctime)s | %(name)s | " "%(lineno)3s | %(message)s"
    ...
    logging.basicConfig(
        level=log_level, format=my_format, datefmt=date_format, filename=os.path.join("logs", f"log.{date_str}")
    )
